...
pyramid = [[int(y) for y in x.split(" ")] for x in raw_text.split("\n")]
...
for row in range(len(pyramid)):
    for col in range(row + 1):
        print(f"({row}, {col})")
        left, right = 0, 0
        # Check previous left choice
        if col > 0:
            left = result[row - 1][col - 1]
        # Check previous right choice
        if col < row:
            right = result[row - 1][col]
        result[row][col] = pyramid[row][col] + max(left, right)
